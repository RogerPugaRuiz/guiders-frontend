    ...
    try {
      ...
      if (user) {
        ...
        try {
          ...
          console.log('[AppInitializer] ✅ Comercial conectado:', commercial.id);
          ...
          console.log('[AppInitializer] 🔄 Habilitando reconexión automática...');
          ...
          if (webSocketService.connected) {
            ...
            webSocketService.welcome$.subscribe((event) => {
              console.log('[AppInitializer] 👋 Bienvenida del servidor:', event.message);
            });

            // Suscribirse a errores
            webSocketService.error$.subscribe((event) => {
              console.error('[AppInitializer] ❌ Error WebSocket:', event.message);
            });
            ...
            console.log('ℹ️  El comercial SOLO recibe eventos presence:changed de:');
            ...
            console.log('═══════════════════════════════════════════════════════════');
            console.log('');
            console.log('📡 [AppInitializer] Uniéndose a sala de presencia personal...');
            ...
            console.log('');
            console.log('✅ [AppInitializer] Proceso de unión a sala de presencia completado');
            console.log('🔔 [AppInitializer] Ahora recibirás eventos presence:changed filtrados');
            console.log('═══════════════════════════════════════════════════════════');
            console.log('');
            ...
            console.log('═══════════════════════════════════════════════════════════');
            console.log('📨 [AppInitializer] INICIALIZAR NOTIFICACIONES GLOBALES');
            console.log('═══════════════════════════════════════════════════════════');
            console.log('📥 [AppInitializer] Cargando chats del comercial...');

            try {
              const chats = await firstValueFrom(chatService.getCommercialChats(user.sub));
              ...
              if (chats.length > 0) {
                ...
                console.log(`📡 [AppInitializer] Uniéndose a ${chatIds.length} salas de chat...`);
                webSocketService.joinMultipleRooms(chatIds);
                console.log(`✅ [AppInitializer] Suscrito a ${chatIds.length} chats para notificaciones en tiempo real`);

                // 6.2 Refrescar contadores de mensajes no leídos
                console.log(`🔄 [AppInitializer] Refrescando contadores de mensajes no leídos...`);
                unreadMessagesService.refreshUnreadCounts(chatIds);
                console.log(`✅ [AppInitializer] Contadores de mensajes no leídos inicializados`);
              } else {
                console.log('ℹ️  [AppInitializer] No hay chats para cargar');
              }

              console.log('═══════════════════════════════════════════════════════════');
              console.log('✅ [AppInitializer] Notificaciones globales inicializadas');
              console.log('🔔 [AppInitializer] Badge y notificaciones funcionarán en todas las rutas');
              console.log('═══════════════════════════════════════════════════════════');
              console.log('');
            } catch (error: any) {
              console.warn('⚠️  [AppInitializer] Error al cargar chats:', error.message);
              // No lanzar error para permitir que la app continúe
            }
          } else {
            console.warn('[AppInitializer] ⚠️ WebSocket no se conectó a tiempo');
          }
        } catch (error: any) {
          console.warn('[AppInitializer] ⚠️ Error al conectar comercial:', error.message);
          // No lanzar error para permitir que la app continúe
        }
      }
    } catch (error: unknown) {
